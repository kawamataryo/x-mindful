...
export function DashboardStats({
  ...
  const gridClass = compact ? "grid grid-cols-1 gap-3" : "grid grid-cols-1 md:grid-cols-3 gap-6";
  ...
  return (
    <div className={`${gridClass} ${marginBottom}`}>
      ...
      <div className={`bg-white rounded-lg ${cardPadding} border border-gray-200`}>
        ...
        <p className="text-xs text-gray-500 mt-1">上限: {settings.dailyLimitMinutes}分</p>
      </div>

      {/* 今日の利用時間カード */}
      <div className={`bg-white rounded-lg ${cardPadding} border border-gray-200`}>
        <h3 className="text-sm font-medium text-gray-600 mb-1">今日の利用時間</h3>
        <p className="text-2xl font-bold text-green-600">{todayUsage?.totalUsedMinutes || 0}分</p>
      ...
      <div className={`bg-white rounded-lg ${cardPadding} border border-gray-200`}>
        ...
        <p className="text-2xl font-bold text-purple-600">{todayUsage?.sessions.length || 0}回</p>
        <p className="text-xs text-gray-500 mt-1">
          平均:{" "}
          {todayUsage?.sessions.length > 0
